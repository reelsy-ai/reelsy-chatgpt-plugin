...
4. For images, infer or ask only for the prompt, aspect ratio, resolution, quality, and requested count. Unless the user explicitly chooses another tier, always use `resolution="1k"` with `quality="low"`; `1k` is the default draft tier, not a request to ask an extra quality question. Only use `2k`/`4k` or `medium`/`high` after the user asks for that upgrade. Use `text_to_image` without references. Use `image_to_image` only with ready owner-scoped Project image Artifacts. The default `1k + low` price is `0.5 credit per image`, so calculate `expectedCredits = count × 0.5`; non-default specifications retain the legacy 2-credit Job price. Show the actual Job count and total cost, obtain explicit confirmation, then call `submit_reelsy_image_generation` with the calculated `expectedCredits` and one stable idempotency key per Job.
5. For source-grounded speech work, use `submit_reelsy_transcription` on a ready Project video or audio Artifact. Show the fixed 1-credit cost and wait for explicit confirmation before calling it with `expectedCredits=1`; preserve the returned `transcript` Artifact with its `words[]` and `segments[]`. Before approved narration, call `list_reelsy_tts_voices` and use only a returned official Gemini voice or legacy alias. Show the exact text, selected voice, and fixed 3-credit cost before calling `submit_reelsy_voice_generation` with `expectedCredits=3`; preserve the returned `voiceover` Artifact. For video planning, convert the request or active Domain Skill result into a compact production intent: objective, aspect ratio, target duration, continuity anchors, ordered beats, audio policy, and completion criteria.
...
7. Poll every asynchronous image or video Job with `get_reelsy_job_status`. Preserve ready `generated_image` and `generated_clip` Artifacts. Transcription and voiceover return ready Artifacts directly; preserve them on the Project Canvas and retry only a failed operation. Never expose Provider errors, tokens, storage keys, or raw Provider URLs.
